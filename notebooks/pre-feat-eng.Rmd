...
main_data <- kNN(main_data, variable = "pm25", k = 5) # can change later to see whihc K gives best model preformace 

# Check that missing values are filled
summary(main_data$pm25)
```
# Getting data ready for Multiple Linear Regression model:
Convert subgroup to ordered factor (for performance ranking) and other categorical variables to factors
```{r}
final_data <- final_data %>%
  mutate(subgroup = factor(subgroup,
                           levels = c("slow","recreational","average","competitive","elite"),
                           ordered = TRUE),
...
Normalization on the continuous predictors (Keeping both sets (Before and after scaling) for the different models)
```{r}
# Find the continuous predictors
continuous_vars <- c(
  "high_temp", "low_temp", "avg_temp", "precipitation",
  "dew_point", "wind_speed", "visibility", "sea_level_pressure",
  "aqi", "co", "ozone", "pm10", "pm25", "no2"
)

# Standardize also knwon as z-score normalize the continuous predictors
final_data_scaled <- final_data %>%
  mutate(across(all_of(continuous_vars), ~ as.numeric(scale(.)), 
                .names = "{.col}_scaled"))

# Check results
summary(final_data_scaled)
```
