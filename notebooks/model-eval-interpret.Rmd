...
for(i in 1:nrow(params_grid)) {
  ...
  params$nthread <- parallel::detectCores()
...
```
The tuned XGBoost model reduces overfitting while maintaining high predictive accuracy, achieving strong performance on both the cross-validation and test sets. It provides a reliable and generalizable solution for this dataset.
...
```
The tuned XGBoost modelis better because it has much lower training, CV, and test errors (RMSE and MAE) and higher R2 values, indicating it fits the data well while still generalizing effectively to unseen data.
