...
We can see that the model is not overfitting severely, only to a reasonable extent (Train RMSE ≈ 43.9, Test RMSE ≈ 137.1). The gap between train and test performance is expected and much smaller than in the baseline model. This indicates that regularization and tuning have helped reduce the degree of overfitting.

We also can see that the cross validation error is very similar to the test error (CV RMSE ≈ 164.7, Test RMSE ≈ 137.1). This is good because it means that the model is stable across folds, the cross-validation process is reliable, and the model is generalizing consistently. This also means that there is no obvious data leakage.

In addition, both R² scores show that the model fits the data extremely well (Train R² ≈ 0.9998, Test R² ≈ 0.9985). The almost perfect R² is likely due to the subgroup structure, rather than overfitting.

Overall, the tuned model clearly outperforms the baseline model. The baseline had a Test RMSE of about 161.6, whereas the tuned model achieves 137.1, which is a meaningful improvement in accuracy. This represents roughly a 15% reduction in test error, showing that the tuning loop successfully improved generalization and stability.
...
comparison_table <- data.frame(
  ...
  Raw_Feature_Model = c(
    43.8937, 33.2543, 0.9998,
    164.6655,
    137.1042, 93.8278, 0.9985
  ),
  Engineered_Feature_Model = c(
    37.4778, 27.8817, 0.9999,
    151.6136,
    142.9413, 95.4914, 0.9983
  )
)
...
The engineered-feature model has a lower training error (Train RMSE ≈ 37.48, Train MAE ≈ 27.88) and slightly better cross-validation performance (CV RMSE ≈ 151.61) compared to the raw-feature model (Train RMSE ≈ 43.89, Train MAE ≈ 33.25, CV RMSE ≈ 164.67). This means that the engineered features help the model fit the training data more accurately and generalize more consistently across folds.

Interestingly, the raw-feature model shows slightly better test performance (Test RMSE ≈ 137.10, Test MAE ≈ 93.83) than the engineered-feature model (Test RMSE ≈ 142.94, Test MAE ≈ 95.49). However, its higher CV RMSE suggests that this improvement might be due to other things, possibly the variability in the specific train-test split. This makes it potentially less stable across different subsets of the data.
...
#BERLIN TEST
Next we tested the model on the Berlin data
```{r}
berlin_data <- final_data %>% filter(marathon == "Berlin")

# Training data 
berlin_data$ozone_bin <- ifelse(berlin_data$ozone >= 38, 1, 0)
berlin_data$ozone_bin <- factor(berlin_data$ozone_bin, levels = c(0, 1))
berlin_data$pm25_bin  <- ifelse(berlin_data$pm25 >= 54, 1, 0)
berlin_data$pm25_bin <- factor(berlin_data$ozone_bin, levels = c(0, 1))


# Remove ozone and pm25 from training and test data
berlin_data <- berlin_data[, !(names(berlin_data) %in% c("ozone", "pm25"))]

#remove visibility, co, pm10, pm25_bin
berlin_data <- berlin_data %>%
  select(-pm25_bin, -pm10, -co, -visibility,-main_pollutant, -ozone_bin)

berlin_data$supershoe = ifelse(berlin_data$year >= 2018, 1, 0)

berlin_data$supershoe <- factor(berlin_data$supershoe)
berlin_data$marathon <- factor(berlin_data$marathon)
berlin_data$gender   <- factor(berlin_data$gender)
berlin_data$subgroup <- factor(berlin_data$subgroup)


# Build test matrix USING training terms
test_matrix <- model.matrix(
  terms(train_matrix),
  data = berlin_test %>% select(-year, -n)
)

# Create DMatrix
dtrain <- xgb.DMatrix(train_matrix, label = berlin_train$avg_chip_seconds)
dtest  <- xgb.DMatrix(test_matrix)


# Convert dataset to numeric-only matrix
berlin_data_matrix <- model.matrix(
  avg_chip_seconds ~ .,
  data = berlin_data %>% select(-year, -n, -marathon)
)

dberlin_test  <- xgb.DMatrix(data = berlin_data_matrix,  label = berlin_data$avg_chip_seconds)
pred_test  <- predict(xgb_model, dberlin_test)


# Berlin Testing metrics
rmse_berlin <- rmse(berlin_data$avg_chip_seconds, pred_train)
mae_berlin <- mae(berlin_data$avg_chip_seconds, pred_train)
rss_berlin <- sum((berlin_data$avg_chip_seconds - pred_train)^2)
tss_berlin <- sum((berlin_data$avg_chip_seconds - mean(berlin_data$avg_chip_seconds))^2)
r2_berlin  <- 1 - rss_train / tss_train



# Combine into a single table
model_eval <- data.frame(
  Metric = c("Berlin RMSE", "Berlin MAE", "Berlin R2"),
  Value  = c(rmse_berlin, mae_berlin, r2_berlin)
)

# Display table
knitr::kable(model_eval, digits = 4, caption = "XGBoost (Engineered Features) Model Best Hyperparameters - Berlin")
```
...
(probably dont need this since i guest the parameters (KB))
