...
Adding the best two bins:
```{r}
# ozone bin: 0 = low, 1 = high
main_data$ozone_bin <- ifelse(main_data$ozone >= 38, 1, 0)

# pm25 bin: 0 = low, 1 = high
main_data$pm25_bin <- ifelse(main_data$pm25 >= 54, 1, 0)

# convert to factors
main_data$ozone_bin <- factor(main_data$ozone_bin, levels = c(0, 1))
main_data$pm25_bin  <- factor(main_data$pm25_bin,  levels = c(0, 1))
...
```{r}
# separate ozone and pm25 bin code
#main_data$ozone_bin <- ifelse(main_data$ozone >= 38, 1, 0)
#main_data$pm25_bin  <- ifelse(main_data$pm25 >= 54, 1, 0)

#main_data$ozone_bin <- factor(main_data$ozone_bin)
#main_data$pm25_bin  <- factor(main_data$pm25_bin)
```

```{r}
# have temp and wind bins here for combination building, can remove and just focus on ozone and pm25
# temp bin
main_data$temp_bin <- ifelse(main_data$avg_temp <= 58, 
                             "Cool or Moderate", 
                             "Warm")
main_data$temp_bin <- factor(main_data$temp_bin)
...
# wind bin
main_data$wind_bin <- ifelse(main_data$wind_speed >= 13, 1, 0)
main_data$wind_bin <- factor(main_data$wind_bin)
```
